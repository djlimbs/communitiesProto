...
App.ViewApplicantsRoute = Ember.Route.extend({
    beforeModel: function() {
        return new Ember.RSVP.Promise(function(resolve, reject) {
            ...
            if (isMobile) {
                var mobileAddress = window.location.href.replace('viewApplicants', 'viewApplicantsMobile').replace('scontrolCaching=1&', '')

                mobileAddress = mobileAddress.substring(0,mobileAddress.indexOf('&appId'));
...
console.log(mobileAddress);
                ...
                console.log(JSON.stringify(objectToBase64Encode))
                cont.convertShareUrlTo64(JSON.stringify(objectToBase64Encode), function(res, evt) {
                    if (res) {
                        ...
                        if (res.isSuccess) {
                            ...
                            window.location.href = window.location.protocol + '//na24.salesforce.com/one/one.app#' + res.data.encodedUrl;
                        } else {
                            // ERROR
                        }
                    } else {
