...
App.LocationModalView = Ember.View.extend({
    ...
    didInsertElement: function() {
        $('#locationModal').on('hidden.jui.modal', function() {
            $('body').css('height', '');
            updateHeight();
        });
    }
...
App.JobPostingController = Ember.ObjectController.extend({
    ...
    redirectUrl : function(){
        ...
        } else if(!Ember.isNone(application) && application.Stage__c === 'Application' && application.Status__c === 'In Progress'){
    ...
    appliedAlertClass: function() {
        ...
        if (!Ember.isNone(application)) {
            ...
            var isCompleted = application.Stage__c !== 'Application' || (application.Stage__c === 'Application' && application.Status__c === 'Completed');
    ...
    disableApplyButton: function() {
        var application = this.get('application')
        if(!Ember.isEmpty(application) && Ember.isEmpty(application.Job_Offers__r) && ((application.Stage__c === 'Application' && application.Status__c == 'Completed') || application.Stage__c !== 'Application')) {
            return 'disabled';
        }
    ...
    apply : function(applyType){
        var self = this;
        var applyUrl = parent.urlPrefix + '/Apply?reqId=' + self.get('jobPosting').Requisition_Lookup__c + '&jobPostingId=' + self.get('jobPosting').Id
        ...
        return new Ember.RSVP.Promise(function(resolve, reject) {
            ...
            jobPostingMap.applications.forEach(function(app) {
                ...
                var applicationObj = {
                    ...
                    statusText: application.Stage__c !== 'Application' || (application.Stage__c === 'Application' && application.Status__c === 'Completed') ? 'Applied' : 'In Progress',
