...
function appIsCompleted(app) {
    return (app.Stage__c === 'Application' && app.Status__c === 'Completed') || app.Stage__c !== 'Application';
};
...
App.JobPostingController = Ember.ObjectController.extend({
    ...
    isJobAppliedCompleted: function(){
        var application = this.get('applications').findBy('jobPostingId', this.get('Id'));

        return !Ember.isEmpty(application) && appIsCompleted(application);
    }.property('Id', 'applications'),

    isJobAppliedInProgress: function(){
        var application = this.get('applications').findBy('jobPostingId', this.get('Id'));

        return !Ember.isEmpty(application) && application.Stage__c == 'Application' && application.Status__c == 'In Progress';
    }.property('Id', 'applications'),
    ...
    model: function(params) {
        ...
        if (!Ember.isEmpty(parsedJobSearchMap.applications)) {
            parsedJobSearchMap.applications.forEach(function(app) {
                ...
                if(app.Outcome__c == 'Rejected'){
                    statusText = 'Not Selected'
                } else if(app.Outcome__c == 'Withdrew'){
                    statusText = 'Withdrew';
                } else if(!Ember.isEmpty(app.Job_Offer_Lookups__r)){
                    app.Job_Offer_Lookups__r.records[0].Status__c
                } else if (appIsCompleted(app)){
                    statusText = 'Applied'
                } else {
                ...
                var applicationObj = { 
                    ...
                    isApplication: true,
                    hasJobOffer: !Ember.isEmpty(app.Job_Offer_Lookups__r),
                    statusText: statusText,
                    Status__c : app.Status__c,
                    Stage__c : app.Stage__c,
                    isRemoteAvailable: app.Requisition_Lookup__c.Allow_Remote_Employees__c,
                    createdDate: app.CreatedDate
                };

                if (!Ember.isEmpty(app.Job_Offer_Lookups__r)) {
                    if (['Accepted', 'Extended'].indexOf(app.Job_Offer_Lookups__r.records[0].Status__c) !== -1) {
